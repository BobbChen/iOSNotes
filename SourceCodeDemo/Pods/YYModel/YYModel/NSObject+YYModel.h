...
/**
 返回模型实例通过传入json数据(字典、字符串、NSData)，线程安全
...
// 当模型中的属性和数据中的key不一致的时候，重写该方法指定映射关系
...
// 如果模型中的属性是字典或者数组，需要在模型的.m中重写该方法，指定数据中的该值对应的类
...
/**
 ...
 @return An array of property's name.
 */
// 如果实现了该方法，则处理过程中不会处理该列表外的属性（只处理这个方法中指定的属性）
 ...
 */
// json转换model的时候调用该方法，可以用来判断数据中的数值是否等于某个值
